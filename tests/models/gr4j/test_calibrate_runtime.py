...
from hydro_agent.models.gr4j.calibrate_runtime import run as calibrate_run

FIXTURES = Path(__file__).resolve().parents[2] / "fixtures" / "gr4j"


def test_gr4j_calibrate_runtime_smoke(tmp_path):
    ...
    # Synthetic observed series close to a zero-exchange GR4J baseline is not
    # required; any finite daily series lets the optimizer finish a tiny budget.
    ...
    assert result["model_id"] == "gr4j"
    assert result["strategy_id"] == "gr4j-bounded-v1"
    assert set(result["candidate_parameters"]) == {"X1", "X2", "X3", "X4"}
