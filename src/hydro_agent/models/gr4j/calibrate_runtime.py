"""GR4J calibration runtime: scientific strategy + pluggable numerical optimizer."""
...
from .contracts import Gr4jScheme
from .conversion import load_gr4j_inputs
from .engine import MODEL_SHA256, MODEL_VERSION, load_param_ranges, simulate
from .param_groups import normalize_param_groups, resolve_param_names
...
def run(workspace: Path) -> dict:
    ...
    if (
        request.model_id != "gr4j"
        or request.capability != "calibrate"
        or request.policy.device != "cpu"
    ):
        raise ValueError("unsupported GR4J calibration")

    strategy_id = str(request.parameters.get("strategy_id") or "gr4j-bounded-v1")
    ...
    scheme, basin, dates, inputs = load_gr4j_inputs(workspace)
    ...
    if not tunable_names:
        raise ValueError("no tunable GR4J parameters resolved from param_groups")
    ...
    if baseline_cached is None:
        ...
        if raw is None:
            raise ValueError("baseline GR4J scheme is not evaluable")
