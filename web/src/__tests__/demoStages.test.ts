...
describe('demo stage mapping', () => {
  it('maps actions to audience stages and keep decision copy', () => {
    expect(stageForAction('A03_FORECAST')).toBe('forecast')
    expect(stageForAction('A05_FORECAST')).toBe('forecast')
    expect(actionTitle('A08_GATE')).toBe(actionTitle('A06_GATE'))
    expect(actionTitle('A06_GATE')).toContain('检查')
    ...
    const statuses = stageStatuses(['A01_CHECK_DATA', 'A03_FORECAST'], 'A05_OPTIMIZE', 'running')
    expect(statuses.data).toBe('done')
    expect(statuses.forecast).toBe('done')
    expect(statuses.gate).toBe('active')
    const finished = stageStatuses(['A01_CHECK_DATA', 'A03_FORECAST', 'A10_EVALUATE_REPORT'], null, 'completed')
