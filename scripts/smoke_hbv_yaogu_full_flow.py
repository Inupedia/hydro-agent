#!/usr/bin/env python3
"""Real Yaogu materials × source_verified HBV-light full research-loop smoke.

Reuses a ready ModelPlan (academy forcing/flow) but binds the task to HBV
...
OUT = ROOT / "artifacts" / "hbv-yaogu-flow-smoke"
...
GR4J_PLAN = ROOT / "artifacts" / "gr4j-yaogu-flow-smoke" / "model-plans"

EXPECTED_PARAMS = set(default_model_registry().get("hbv").descriptor.parameter_names)
...
def resolve_ready_plan(plans: ModelPlanService) -> dict:
    for legacy in (LEGACY_PLAN, GR4J_PLAN):
        if not legacy.is_dir():
            continue
        for path in sorted(legacy.glob("plan-*/plan.json")):
...
def main() -> int:
    ...
    plugin = default_model_registry().get("hbv")
    if (
        plugin.descriptor.validation_status != "source_verified"
        or not plugin.descriptor.supports_calibration
    ):
        raise RuntimeError("HBV must be source_verified before this product-path smoke")
    ...
    if "calibrate" not in view0.model.capabilities:
        raise RuntimeError("product path must advertise calibrate for source_verified HBV")
    if set((scheme.config_json or {}).get("parameters") or {}) != EXPECTED_PARAMS:
        raise RuntimeError("HBV task must bind HBV-light defaults, not another model")
    ...
    for _ in range(20):
        ...
        raise RuntimeError("HBV scientist did not close out within smoke resource limit")
    ...
    if missing:
        raise RuntimeError(f"incomplete HBV path, missing actions: {missing}")
    if not any(p.action == ActionCode.A05_OPTIMIZE and p.status == "succeeded" for p in packets):
        raise RuntimeError("HBV formal calibration did not succeed")
    print("OK hbv yaogu product-path smoke completed", flush=True)
    return 0
